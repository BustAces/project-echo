...
const MenuItem: React.FC<React.PropsWithChildren<FooterProps>> = ({
  ...
  return (
    ...
    >
      <Flex flexDirection="column" width={["100%", null, "1250px;"]} px={["16px", "24px"]} mx="auto">
          ...
          {items?.map((item) => (
            <StyledList key={item.label}>
              ...
              {item.items?.map(({ label, href, isHighlighted = false }) => (
                <StyledListItem key={label} style={{ marginBottom: "3px" }}>
                  {href ? (
                    <Link
                      data-theme="dark"
                      href={href}
        ...
        >
          ...
          <Flex order={[1, null, 2]} mb={["24px", null, "0"]} justifyContent="space-between" alignItems="center">
            <Box mr="20px">
              <KazamaPrice chainId={chainId} cakePriceUsd={cakePriceUsd} color="textSubtle" />
            </Box>
            <Button
              data-theme="dark"
              as="a"
