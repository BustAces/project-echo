...
import { kazamaBaseColors } from "../../../tokens";
...
const StyledPanel = styled.div<{ isPushedRight: boolean; showChat: boolean; showMenu: boolean }>`
  ...
  flex-shrink: 0;
  background: ${kazamaBaseColors.kazamaPanels};
  height: 93%;
  margin-top: 77px;
  transition: padding-top 0.2s, width 0.2s cubic-bezier(0.4, 0, 0.2, 1);
  border-left: ${({ isPushedRight }) => (isPushedRight ? "1px solid rgba(0, 0, 0, 0.089)" : 0)};
...
const PanelRight: React.FC<Props> = (props) => {
  ...
  return (
    <StyledPanel
      style={{ width: `${isPushedRight ? SIDEBAR_RIGHT_WIDTH_FULL : SIDEBAR_RIGHT_WIDTH_REDUCED}px` }}
