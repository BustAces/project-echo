...
import { SearchIcon, CloseIcon, ChevronUpIcon, ChevronDownIcon, Image, ChevronFilledIcon } from '@kazama-defi/uikit'
...
const UserAvatar = styled.img`
  // Create a styled component for the user avatar
  width: 20px;
  height: 20px;
  border-radius: 50%;
  margin-right: 10px;
...
const OnlineUsers: React.FC = () => {
  ...
  useEffect(() => {
    const filtered = onlineUsersData.onlineUsers.filter((user) =>
      searchQuery && searchQuery.length > 0 ? user.username.toLowerCase().includes(searchQuery.toLowerCase()) : true,
    )
        ...
        <OnlineWrapper>
          <div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <div>
                <Image src={`/images/chat/online.svg`} width={24} height={24} />
              </div>
              ...
              <Selectbox>
                {sortedFilteredUsers.map((user) => (
                  <div
                    key={user._id}
                    className="userrow"
                    style={{ justifyContent: 'space-between', display: 'flex', fontSize: '13px' }}
                  >
                      ...
                      {user.role === 'admin' ? (
                        ...
                        <p
                          style={{
                            marginBottom: '2px',
                            fontSize: '0.9rem',
                            color: 'rgb(25, 197, 255)', // Different color for mod
                          }}
                        >
                          {user?.username.length > 20 ? truncateHash(user?.username) : user?.username}
                        </p>
                        ...
                        <p
                          style={{
                            marginBottom: '2px',
                            fontSize: '15px',
                            color: '#fff', // Different color for user
                            fontFamily: 'Industry-Black',
                          }}
                        >
                          {user?.username.length > 20 ? truncateHash(user?.username) : user?.username}
