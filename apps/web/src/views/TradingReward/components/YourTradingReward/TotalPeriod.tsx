...
import { Box, Flex, Card, Text, Message, MessageText, TooltipText, Button, useTooltip } from '@kazama-defi/uikit'
...
const TotalPeriod: React.FC<React.PropsWithChildren<TotalPeriodProps>> = ({
  ...
  const notReadyForClaim = useMemo(() => {
    ...
    return totalAvailableClaimData.filter((campaign) => {
      ...
      if (!campaignIncentive.is_activated) {
        return campaign
      }
    })
    ...
    return (
      totalAvailableClaimData
        ...
        .filter((campaign) => {
          ...
          const isValid =
            campaignIncentive.is_activated &&
