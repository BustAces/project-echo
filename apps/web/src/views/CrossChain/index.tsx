    ...
    <>
      <Page style={{ marginTop: '64px', maxWidth: '1100px' }}>
        <CurrencyContainer style={{ marginBottom: '10px' }}>
          <ChartContainer>{selectedFrom && <FromChart ticker={selectedFrom.name.toLowerCase()} />}</ChartContainer>
          <SwapContainer>
            <div style={{ display: 'flex', flexDirection: 'column', background: '#21252b', borderRadius: '0.25rem' }}>
              <div style={{ padding: '20px' }}>Select sending currency</div>
              <div style={{ background: '#21252b', padding: '20px', margin: '15px', borderRadius: '0.25rem' }}></div>
              <div>2</div>
            </div>
          </SwapContainer>
        </CurrencyContainer>
        ...
        <CurrencyContainer>
          <ChartContainer>{selectedTo && <FromChart ticker={selectedTo.name.toLowerCase()} />}</ChartContainer>
          <SwapContainer>3</SwapContainer>
        </CurrencyContainer>
      </Page>
      ...
      <Page style={{ marginTop: '64px' }}>
        ...
        <CurrencyContainer>
          <div style={{ flexDirection: 'row' }}>
            <div>{selectedFrom && <FromChart ticker={selectedFrom.name.toLowerCase()} />}</div>
            <div>{selectedFrom && <FromChart ticker={selectedTo.name.toLowerCase()} />}</div>
          </div>
          {selectedFrom && selectedFrom.name}
          <div></div>
          ...
          <TradeBox>
            ...
            {exchangeAmount !== null && exchangeAmount < (minExchangeAmount || 0) && (
              <p style={{ color: 'red' }}>
                Exchange amount must be greater than or equal to the minimum exchange amount.
              </p>
            )}
            ...
            {estimatedAmount !== null && (
              <div>
                <p>
                  Estimated Exchange Amount: {estimatedAmount} {selectedTo?.ticker.toUpperCase()}
                </p>
            ...
            {confirmationChecked && (
              <div>
                ...
                <input type="text" id="extraId" value={extraId} onChange={(e) => setExtraId(e.target.value)} />
              </div>
            )}
            ...
            <button disabled={!confirmationChecked} onClick={handleCreateExchangeTransaction}>
              Proceed
            </button>
        ...
        {minExchangeAmount !== null && (
          <div>
            <p>
              Minimum Exchange Amount: {minExchangeAmount || 0} {selectedFrom?.ticker.toUpperCase()}
            </p>
          </div>
        )}
      </Page>
