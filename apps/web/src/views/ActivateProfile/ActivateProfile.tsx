  ...
  const BalanceMessage = () => {
    if (kazamaBalance >= KAZAMA_TO_BURN) {
      ...
      return (
        <Flex
          style={{
            background: 'rgba(16, 217, 96, 0.74)',
            border: '1px solid #10D960',
            borderRadius: '7px',
        ...
        >
          <Text textAlign="center" fontSize="12px" color="#fff">
            {' '}
            Check passed
          </Text>
        </Flex>
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <StyledResultWrapper>
          <Indicator marginTop="10px" marginBottom="10px">
            {pendingTx && (
              <>
                <Dots style={{ fontSize: '20px' }}>Confirming</Dots>
              </>
            )}
      ...
      <Flex>
        <SetNameButton
          ...
          onClick={async () => {
            ...
            if (receipt?.status) {
              toastSuccess(`${t('Burned successfully ..')}!`)
              setIsBurned(true)
              handleConfirm()
            }
          }}
