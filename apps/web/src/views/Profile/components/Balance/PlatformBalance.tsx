...
import { Flex, PaperStackIcon, ChevronFilledIcon, BscNetworkIcon } from '@kazama-defi/uikit'
...
const PlatformBalance: React.FC = () => {
  ...
  // Ensure the balance is a number and format it to two decimal places
  const formattedBalance = new Intl.NumberFormat('en-US', {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  }).format(Number(userData.platform_balance.balance) || 0)

  return (
    <Container>
      <BalanceBox onClick={() => setIsOpen(!isOpen)} style={{ fontSize: '14px', fontFamily: 'Flama Bold' }}>
        <BscNetworkIcon width={32} mr="5px" />
        {formattedBalance}
