...
import { NATIVE, WNATIVE } from '@kazama-defi/sdk'
import { ChainId } from '@kazama-defi/chains'
import { FetchStatus } from 'config/constants/types'
import { getFullDisplayBalance, formatBigIntToFixed } from '@kazama-defi/utils/formatBalance'
...
const Dropdown = styled.div<{ isOpen: boolean }>`
  ...
  box-shadow: rgba(0, 0, 0, 0.25) 0px 5px 8px -4px, rgba(0, 0, 0, 0.18) 0px 0px 20px 0px,
...
const EthNetwork = styled(EthereumIcon)`
  width: 32px;
  margin-right: 5px;
`
...
const WalletBalance: React.FC = () => {
    ...
    const fetchTokenPrices = async () => {
      try {
        const response = await axios.get(
          'https://api.coingecko.com/api/v3/simple/price?ids=binancecoin,usd-coin,tether,bitcoin,ethereum,dai,apecoin,shiba-inu&vs_currencies=usd&include_24hr_change=true&include_price_change=true',
        )
  ...
  const formattedTotalValueWithChange = `${formatDollarAmount(
    totalValueWithChange,
  )} / ${valueChangeSign}${formatDollarAmount(Math.abs(totalValueChange))}`
  ...
  const MarketBox = styled.div<{ change: number }>`
    background: ${(props) =>
      props.change > 0 ? 'rgba(16, 217, 96, 0.233)' : props.change < 0 ? 'rgba(255, 73, 73, 0.253)' : null};
    border-radius: 3px;
    ...
    color: ${(props) => (props.change > 0 ? 'rgb(16, 217, 96)' : props.change < 0 ? 'rgb(255, 73, 73)' : 'orange')};
  `
    ...
    <Container>
      ...
      {isOpen && (
        <Dropdown ref={dropdownRef} isOpen={isOpen}>
          <div style={{ fontFamily: 'Industry-Black', fontSize: '18px', padding: '20px' }}>YOUR ASSETS</div>
          <DataContainer>
            <TokenContainerOdd>
              <LogoDiv>
              ...
              <MarketChangesDiv>
                <div>
                  <MarketBox change={tokenPrices['binancecoin'].usd_24h_change}>
                    {GetIcon(tokenPrices['binancecoin'].usd_24h_change)}{' '}
                    {tokenPrices['binancecoin'].usd_24h_change.toFixed(2)}%
                  </MarketBox>
                </div>
              </MarketChangesDiv>
              <BalanceDiv>
              ...
              <MarketChangesDiv>
                <div>
                  <MarketBox change={tokenPrices['ethereum'].usd_24h_change}>
                    {GetIcon(tokenPrices['ethereum'].usd_24h_change)}{' '}
                    {tokenPrices['ethereum'].usd_24h_change.toFixed(2)}%
                  </MarketBox>
                </div>
              </MarketChangesDiv>
              <BalanceDiv>
              ...
              <MarketChangesDiv>
                <div>
                  <MarketBox change={tokenPrices['usd-coin'].usd_24h_change}>
                    {GetIcon(tokenPrices['usd-coin'].usd_24h_change)}{' '}
                    {tokenPrices['usd-coin'].usd_24h_change.toFixed(2)}%
                  </MarketBox>
                </div>
              </MarketChangesDiv>
              <BalanceDiv>
              ...
              <MarketChangesDiv>
                <div>
                  <MarketBox change={tokenPrices['shiba-inu'].usd_24h_change}>
                    {GetIcon(tokenPrices['shiba-inu'].usd_24h_change)}{' '}
                    {tokenPrices['shiba-inu'].usd_24h_change.toFixed(2)}%
                  </MarketBox>
                </div>
              </MarketChangesDiv>
              <BalanceDiv>
