...
const MessageWall: React.FC<{ address: string }> = ({ address }) => {
  ...
  useEffect(() => {
    const fetchWallData = async () => {
      try {
        ...
        const sortedUnpinnedMessages = unpinnedMessages.sort(
  ...
  const postMessage = async (
    messageText: string,
    pollData: { main_question: string; options: any[] } | null = null,
  ) => {
    try {
  ...
  const submitMessage = () => {
    if (isCreatingPoll) {
      const pollData = {
        ...
        options: pollOptions
          .map((optionText) => ({ text: optionText, votes: 0 }))
  ...
  const newMessage = () => {
    return (
      <CreateContainer>
        ...
        <div>
          {characterCount > 2000 ? (
            <WarningText>{characterCount} / 2000</WarningText>
          ) : (
            <Text>{characterCount} / 2000</Text>
          )}
        </div>
        {isCreatingPoll ? (
          <>
            ...
            <Button onClick={submitMessage}>Post Message with Poll</Button>
            <Button onClick={cancelPoll}>Cancel</Button>
          </>
        ) : (
          <Button onClick={() => postMessage(newMessageText)}>Post Message</Button>
        )}
        <OptionsBox>
          <IconBox>
  ...
  return (
    <WallContainer>
      {owner ? <div style={{ width: '100%', marginBottom: '5px' }}>{newMessage()}</div> : null}
      {messageWall?.messages.length ? (
        ...
        <NoContent>No messages on timeline</NoContent>
      )}
    </WallContainer>
  )
}
