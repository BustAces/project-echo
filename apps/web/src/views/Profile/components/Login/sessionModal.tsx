...
import { Input as UIKitInput, Modal, Button, Text } from '@kazama-defi/uikit'
...
const ActivateSession: React.FC<React.PropsWithChildren<any>> = ({ onDismiss }) => {
  ...
  // Changeable vars
  let imagePath = `${process.env.NEXT_PUBLIC_KAZAMA_RANK_IMAGE_PATH}/unranked_.png`
  ...
  if (userData.rank_data.level === 0) {
    imagePath = `${process.env.NEXT_PUBLIC_KAZAMA_RANK_IMAGE_PATH}/unranked_.png`
    ...
    <StyledModal title={t('ACCOUNT LOGIN')} onDismiss={onDismiss}>
      ...
      <div>
        <ProgressBar
          ...
          customLabel="XP"
          isLabelVisible={true}
          completed={userData.rank_data.rank_progress}
          maxCompleted={100}
        />
      </div>
      <div style={{ marginBottom: '10px', textAlign: 'center' }}>
        <Text fontFamily="Flama Bold" style={{ color: '#fff' }}>
          This account has activated extra password security, enter the password to use all profile and social related
          components.
        </Text>
      </div>
      <div>
        <Text fontSize="15px">
          It is not required to log in with your password, you can always interact with our defi products. However, you
          will not be able to interact with the social aspects of the platform or change profile settings.
        </Text>
      </div>
      <Input
        type="password"
