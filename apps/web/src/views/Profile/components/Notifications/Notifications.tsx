...
const NotificationDot = styled.div`
  position: absolute;
  background-image: linear-gradient(#ee6a35, #ee5519);
  ...
  z-index: 1000;
  left: 3px;
  animation: notipulse 1.7s infinite;
  @keyframes notipulse {
    0% {
      transform: scale(0.95);
      box-shadow: 0 0 0 0 rgb(238, 106, 53);
    }
    ...
    <Container>
      ...
      {isOpen && (
        <Dropdown ref={dropdownRef} isOpen={isOpen}>
          <DropdownTop>
            <Text style={{ fontSize: '15px', fontFamily: 'Industry-Black' }}>NOTIFICATIONS</Text>
            <CloseImage onClick={() => setIsOpen(false)} />
          </DropdownTop>
          <NotiStorageDiv>
            <ProgressBar
              className="glowing-progress-bar"
              baseBgColor="rgba(79, 91, 142, 0.363)"
                    ...
                    <HeaderText>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                          {!notification.marked_as_readed ? <NewBadge>NEW</NewBadge> : null}
                          <div style={{ fontFamily: 'Industry-Black' }}>
                            {getNotificationInfo(notification.type).text}
                          </div>
                        </div>
