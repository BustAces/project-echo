...
import { Heading, Text, ButtonMenu, ButtonMenuItem } from '@kazama-defi/uikit'
...
const ProfileHeader = styled.div`
  ...
  align-items: center;
  background: ${({ image }) =>
    image !== 'https://assets.kazama.io/profiles/profile_background/default.jpg'
      ? `linear-gradient(to right, rgba(26, 30, 35, 0.562), transparent 50%, rgba(26, 30, 35, 0.562)), linear-gradient(to left, rgba(26, 30, 35, 0.562), transparent 50%, rgba(26, 30, 35, 0.562)), url(${image})`
      : `url(${image})`};
  ...
  return (
    <Tabs>
      <ButtonMenu scale="sm" variant="subtle" onItemClick={handleClick} activeIndex={view} fullWidth>
        <ButtonMenuItem>Overview</ButtonMenuItem>
        <ButtonMenuItem>Messages</ButtonMenuItem>
        <ButtonMenuItem>Videos</ButtonMenuItem>
        <ButtonMenuItem>NFT</ButtonMenuItem>
        <ButtonMenuItem>Groups</ButtonMenuItem>
        <ButtonMenuItem>Followers</ButtonMenuItem>
        <ButtonMenuItem>Following</ButtonMenuItem>
        <ButtonMenuItem>Stats</ButtonMenuItem>
      </ButtonMenu>
    </Tabs>
  )
}

const Profile = ({ initialView = ProfileView.OVERVIEW }) => {
  // Constants
  ...
  const postNotification = async (type, identifier) => {
    try {
      ...
      const notificationMessage =
        type === 'follow' ? `${spectatorDisplay} is now following you` : 'Someone has unfollowed you.'
  ...
  return (
    <>
      <ProfilePage>
        <ProfileHeader image={profileData.profile_data.header_image}>
          <AvatarSection>
            ...
            <UsernameWrapper>
              <AccountText>{nameDisplay}</AccountText>
              <MiscText>Joined {formatJoinedDate(profileData.joined_at)}</MiscText>
            </UsernameWrapper>
          </AvatarSection>
        ...
        <HeaderBar>
          {(view === ProfileView.MESSAGE_WALL || view === ProfileView.OVERVIEW || view === ProfileView.FOLLOWERS) && (
      ...
      {profileData.rank_data.rank_progress}
        ...
        <ProfileContainer>
          {view === ProfileView.OVERVIEW && <div>ddd</div>}
          {view === ProfileView.MESSAGE_WALL && <MessageWall address={profileData.address} switchView={handleClick} />}
          {view === ProfileView.FOLLOWERS && (
            <FollowersList
              followersList={profileData.profile_data.followers}
