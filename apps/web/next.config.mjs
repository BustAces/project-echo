...
import smartRouterPkgs from '@kazama-defi/smart-router/package.json' assert { type: 'json' }
...
const workerDeps = Object.keys(smartRouterPkgs.dependencies)
  .map((d) => d.replace('@kazama-defi/', 'packages/'))
...
const config = {
  ...
  async headers() {
    return [
      ...
      {
        source: '/assets/fonts/FlamaBold/:all*',
        headers: [
          {
            key: 'Cache-Control',
            value: 'public, immutable, max-age=31536000',
      ...
      {
        source: '/assets/fonts/Industry-Black/:all*',
        headers: [
          {
            key: 'Cache-Control',
            value: 'public, immutable, max-age=31536000',
      ...
      {
        source: '/assets/fonts/fontsBai-Jamjuree/:all*',
        headers: [
          {
            key: 'Cache-Control',
            value: 'public, immutable, max-age=31536000',
